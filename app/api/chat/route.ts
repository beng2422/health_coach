...
export async function POST(req: Request) {
  ...
  if (type === 'daily_message') {
    ...
    2. Estimated calories consumed, burned, and maintenance level based on their profile
    3. Suggestions for improvement
    Format the response in clear sections.
    Also provide a JSON object with nutrition estimates in this format:
    {"calories": number, "protein": number, "carbs": number, "fats": number, "calories_burned": number, "calories_maintenance": number}`
